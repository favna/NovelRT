...
namespace NovelRT::Windowing {
  WindowingService::WindowingService(NovelRunner* const runner) :
    _window(std::unique_ptr<GLFWwindow, decltype(&glfwDestroyWindow)>(nullptr, SDL_DestroyWindow)),
  ...
  void WindowingService::initialiseWindow(int displayNumber, const std::string& windowTitle) {
    ...
    if(!glfwInit())
    {
      _logger.logError("Could not initialize GLFW: ", std::string(glfwGetError()));
    }
    ...
    */
      auto window = glfwCreateWindow(wData,hData,_windowTitle);
